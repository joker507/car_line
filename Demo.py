...
def select_rgb_lines(image,colors):
    ...
    return cv2.bitwise_and(image, image, mask=all_mask)
#
# def selecct_rgb_white_yellow(image,color1_lower=[200, 200, 200],color1_upper=[255, 255, 255],color2_lower=[0, 110, 110],color2_upper=[100, 255, 255]):
#     # cv中读取顺序是BGR
#     # 白色RGB
#     lower = np.uint8([200, 200, 200])
#     upper = np.uint8([255, 255, 255])
#     white_mask = cv2.inRange(image, lower, upper)  # 在范围的就为255 else 0
#     Show('white_mask',white_mask)
#
#     # 黄色
#     lower = np.uint8([0, 110, 110])
#     upper = np.uint8([100, 255, 255])
#     yellow_mask = cv2.inRange(image, lower, upper)  # 在范围的返回白色else 0
#     Show('yellow_mask',yellow_mask)
#
#     # 联合
#     mask = cv2.bitwise_or(white_mask, yellow_mask)  # 白色和黄色通道保留
#     masked = cv2.bitwise_and(image, image, mask=mask)  # 加到原图
#     return masked
...
def select_region(image):
   ...
   选择RIO区域
    ...
    img = cv2.polylines(img,[pts],isClosed=False,color=255,thickness=10) #单通道图片
...
class Car_lines():
    ...
    def search(self,img,colors = [[[200, 200, 200],[255, 255, 255]],[[0, 110, 110],[100, 255, 255]]]):
        lines = select_rgb_lines(img,colors)  # 颜色选择
        Show('slect_liness',lines)

        gray = convert_gray_scale(lines)  # 转换灰度图
        Show('gray',gray)

        smooth_gray = apply_smoothing(gray)  # 高斯平滑
        Show('gauss',smooth_gray)
...
if __name__ == '__main__':
    ...
    if cap.isOpened():
        ret, frame = cap.read()
        if ret:
            result,data = search.search(frame) #调用寻找图片中的车道线
            print(data)
            out.write(result)  # 保存视频
            Show("frame", result)  # 显示图片
